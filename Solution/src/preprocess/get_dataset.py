...
class DataPreprocessing():
    ...
    def process(self):
        ...
        nor = None
        if self.flag_nor:
            # co_col.extend(col_label.to_list())
        ...
        if self.flaq_save:
            self.data_all = pd.concat([self.train_data, self.val_data, self.test_data], axis=0)
            self.data_all.to_csv('./data_preprocessed.csv', index=False, index_label='subject_id')
        ...
        col_all = co_col + ca_col + self.col_label
...
class MyDataset(data.Dataset):
    def __init__(self,
        ...
        if label is not None:
            for index_, values in data.iterrows():
